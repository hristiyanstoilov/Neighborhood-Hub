...
| Admin unpublish doesn't notify event attendees / drive pledgers | UX | Easy | Low | S | ✅ | When admin cancels an event via report unpublish, attendees receive no notification. Same for drives: pledgers are not notified. Contrast with organizer cancel which fires `createNotification` per attendee. Add notification loop in `admin/reports PATCH` for `event` and `drive` target types. |
