  ...
  async function runAction(reservationId: string, action: 'approve' | 'reject' | 'cancel' | 'picked_up', cancellationReason?: string) {
    ...
    try {
      ...
      const messages: Record<string, string> = {
        ...
        DUPLICATE_ACTIVE_RESERVATION: 'You already have an active reservation for this listing.',
        RESERVATION_ALREADY_TERMINAL: 'This reservation has already been completed.',
      }
      setError(messages[code] ?? 'Something went wrong.')
    } finally {
      setBusyId(null)
        ...
        {myReservation?.status === 'pending' || myReservation?.status === 'reserved' ? (
          ...
          <form onSubmit={submitReservation} className="space-y-4">
            ...
            <div>
              ...
              <input type="datetime-local" value={pickupAt} onChange={(e) => setPickupAt(e.target.value)} min={new Date().toISOString().slice(0, 16)} className="w-full border border-gray-300 rounded-md px-3 py-2 text-sm" required />
            </div>
            <div>
